...
class Trader:
    def __init__(self):
        self.prices = {"PRODUCT1": [9], 
                       ...
                       "AMETHYSTS" : [10000],
                       "STARFRUIT" : [],
                       "ORCHIDS" : []}  # Initialize prices for each symbol
        ...
        self.sunlight = {"ORCHIDS": []}  # Sunlight history
        self.humidity = {"ORCHIDS": []}  # Humidity history
    ...
    def hum_miss(self, humidity):
        val = max(humidity - 80, 0) + max(60 - humidity, 0)
        return val
        
    def sun_miss(self, sunlight):
        val = 2556 - sunlight
        return val
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in ["ORCHIDS"]:
            ...
            sun = state.observations.conversionObservations[product].sunlight
            hum = state.observations.conversionObservations[product].humidity
            ...
            if timestamp > 0:
                ...
                self.sunlight[product].append(sun)
                self.humidity[product].append(hum)
                ...
                self.sunlight[product].extend([sun, sun, sun, sun, sun, sun, sun])
                self.humidity[product].extend([hum, hum, hum, hum, hum, hum, hum])
                
            humidity_miss_diff_m5 = (self.hum_miss(self.humidity[product][-5]) - self.hum_miss(self.humidity[product][-6]))
            humidity_miss_diff_m4 = (self.hum_miss(self.humidity[product][-4]) - self.hum_miss(self.humidity[product][-5]))
            humidity_miss_diff_m3 = (self.hum_miss(self.humidity[product][-3]) - self.hum_miss(self.humidity[product][-4]))
            humidity_miss_diff_m2 = (self.hum_miss(self.humidity[product][-2]) - self.hum_miss(self.humidity[product][-3]))
            humidity_miss_diff_m1 = (self.hum_miss(self.humidity[product][-1]) - self.hum_miss(self.humidity[product][-2]))
            
            humidity_miss_diff_diff_m4 = humidity_miss_diff_m4 - humidity_miss_diff_m5
            
            sunlight_diff_m5 = (self.sun_miss(self.sunlight[product][-5]) - self.sun_miss(self.sunlight[product][-6]))
            sunlight_diff_m4 = (self.sun_miss(self.sunlight[product][-4]) - self.sun_miss(self.sunlight[product][-5]))
            sunlight_diff_m3 = (self.sun_miss(self.sunlight[product][-3]) - self.sun_miss(self.sunlight[product][-4]))
            sunlight_diff_m2 = (self.sun_miss(self.sunlight[product][-2])- self.sun_miss(self.sunlight[product][-3]))
            sunlight_diff_m1 = (self.sun_miss(self.sunlight[product][-1]) - self.sun_miss(self.sunlight[product][-2]))
            
            humidity_miss_mean = (humidity_miss_diff_m5) ##+ humidity_miss_diff_m4 + humidity_miss_diff_m3 + humidity_miss_diff_m2 + humidity_miss_diff_m1)/5
            sunlight_miss_mean = (sunlight_diff_m1) ##+ sunlight_diff_m4 + sunlight_diff_m3 + sunlight_diff_m2 + sunlight_diff_m1)/5
            
            humidity_exp_coeff = 0.004
            humidity_factor = math.exp(humidity_exp_coeff * humidity_miss_mean)
            
            sunlight_exp_coeff = -(0.04/60.857)
            sunlight_factor = math.exp(sunlight_exp_coeff * sunlight_miss_mean)
            
            valuation_coeff = humidity_factor * sunlight_factor
            valuation_coeff = valuation_coeff * valuation_coeff
            
            #### NEW HUMIDITY
            
            valuation_coeff = math.exp(-0.1783 * humidity_miss_diff_m5 - 0.2035 * humidity_miss_diff_diff_m4)
            
            ###
            
            # We sell when valuation coeff is good
            
            buy_flag = False
            sell_flag = False
            
            if valuation_coeff < 0.9995:
                sell_flag = True
            else:
                sell_flag = False
                
            if valuation_coeff > 1.0005:
                buy_flag = True
            else:
                buy_flag = False
            ...
            print("Valuation coeff: ", valuation_coeff)
            ...
            for ask, vol in sell_orders.items():
                ...
                if  (ask + sell_cost < conversion_bid * valuation_coeff) and rem_buy > 0 and vol < 0:
            ...
            for bid, vol in buy_orders.items():
                ...
                if  (bid + buy_prize > conversion_ask * valuation_coeff) and rem_sell > 0 and vol > 0:
            ...
            print("NEW POSITION AFTER TRADES IS::::: ", new_position)
            
            if new_position > 90:
                print("!!!! POSITION NEAR LIMIT MUST CONVERT !!!!!")
                print("!!!! Conversions so far: !!!!!", conversions)
                conv = -10
                conversions += conv
                print("END OF ITER CONVERSION:", "Product:", product, "Amount:", conv)
                ...
                if new_position > 0 and valuation_coeff < 1:
                    conv = -min(new_position, max_conversion)
                    conversions -= conv
                    print("END OF ITER CONVERSION:", "Product:", product, "Amount:", conv)
                    
                if new_position < 0 and valuation_coeff > 1:
                    conv = min(max(-new_position-10, 0), max_conversion)
                    conversions += conv
                    print("END OF ITER CONVERSION:", "Product:", product, "Amount:", conv)
