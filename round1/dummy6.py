...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            sma = sum(self.prices[product][-50:]) / len(self.prices[product][-50:])
            ...
            if len(order_depth.sell_orders) != 0:
                ...
                if int(best_ask) < acceptable_price:
                    clipped_best_ask_amount = min(-best_ask_amount, remaining_buy_position)
            ...
            if len(order_depth.buy_orders) != 0:
                ...
                if int(best_bid) > acceptable_price:
                    clipped_best_bid_amount = max(-best_bid_amount, -remaining_sell_position)
            ...
            if product == "AMETHYSTS" and positions < max_position and positions > -max_position:
                product_orders.append(Order(product, 9998, remaining_buy_position - 2*math.floor(remaining_buy_position/3)))
                product_orders.append(Order(product, 9997, math.floor(remaining_buy_position/3)))
                product_orders.append(Order(product, 9996, math.floor(remaining_buy_position/3)))
                
                product_orders.append(Order(product, 10002, -(remaining_sell_position - 2*math.floor(remaining_sell_position/3))))
                product_orders.append(Order(product, 10003, -math.floor(remaining_sell_position/3)))
                product_orders.append(Order(product, 10004, -math.floor(remaining_sell_position/3)))
