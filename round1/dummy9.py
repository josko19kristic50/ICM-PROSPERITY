    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "AMETHYSTS":
                ...
                val1buy = math.floor(rem_buy/3)
                val2buy = math.floor(rem_buy/3)
                
                val1sell = math.floor(rem_sell/3)
                val2sell = math.floor(rem_sell/3)
                
                # Place maker buy order for "AMETHYSTS"
                if position < max_position:
                    product_orders.append(Order(product, 9997, min(val1buy, rem_buy, max_size)))
                    product_orders.append(Order(product, 9998, min(val2buy, rem_buy - val1buy, max_size)))
                    product_orders.append(Order(product, 9996, max(rem_buy - val1buy - val2buy, 0)))

                # Place maker sell orders for AMETHYSTIS
                if position > -max_position:
                    product_orders.append(Order(product, 10003, -min(val1sell, rem_sell, max_size)))
                    product_orders.append(Order(product, 10002, -min(val2sell, rem_sell - val1sell, max_size)))
                    product_orders.append(Order(product, 10004, -min(max(rem_sell - val1sell - val2sell, 0), max_size))) 
