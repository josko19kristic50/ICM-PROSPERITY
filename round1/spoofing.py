...
class Trader:
    ...
    def make_prediction_simple(self, mid_price, mid_price_1_rows_earlier, mid_price_2_rows_earlier,
                                  mid_price_3_rows_earlier, mid_price_4_rows_earlier):
        # Coefficients
        coefficients = {
            'const': 19.2661,
            'mid_price': 0.3149,
            'mid_price_1_rows_earlier': 0.2481,
            'mid_price_2_rows_earlier': 0.1574,
            'mid_price_3_rows_earlier': 0.1535,
            'mid_price_4_rows_earlier': 0.1222
        }
    ...
    def make_prediction_ratios(self, bid_price_1, bid_volume_1, ask_price_1, ask_volume_1,
                    bid_price_2, bid_volume_2, ask_price_2, ask_volume_2,
                    rate_bid_2, rate_ask_2):
        # Coefficients
        coefficients = {
            'const': 5.9282,
            'bid_price_1': -0.0422,
            ...
            'ask_price_2': 0.6023,
            'ask_volume_2': -0.0183,
            'rate_bid_2': 2.866e-06,
            'rate_ask_2': -4.269e-07
        }
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "STARFRUIT":
                ...
                acceptable_price = self.make_prediction_simple(mid_price_1, mid_price_2, mid_price_3, mid_price_4, mid_price_5)
            ...
            if product == "AMETHYSTS":
             ...
             ### BUY ###
                ...
                if new_position < 0 and rem_sell > 0 and rem_buy > 0:
                    product_orders.append(Order(product, 9998, -1))
                    product_orders.append(Order(product, 9997, min(rem_buy, 15)))
                    rem_sell -= 1
                    rem_buy -= rem_buy
                    
                if new_position >= 0 and rem_sell > 0 and rem_buy > 0:
                    product_orders.append(Order(product, 10004, 1))
                    product_orders.append(Order(product, 10003, -min(rem_sell, 15)))
                    rem_sell -= rem_sell
                    rem_buy -= 1
                ...
                if new_position > 0 and rem_sell > 0 and rem_buy > 0:
                    product_orders.append(Order(product, 10004, 1))
                    product_orders.append(Order(product, 10003, -rem_sell))
                    rem_sell -= rem_sell
                    rem_buy -= 1
