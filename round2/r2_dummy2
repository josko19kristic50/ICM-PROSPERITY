...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in ["ORCHIDS"]:
            ...
            conversion_buy_costs = import_tariff + transport_fees # NEGATIVE
            conversion_sell_costs = export_tariff + transport_fees # POSITIVE
            ...
            if position > -20:
                product_orders.append(Order(product, best_bid, -position - max_size))
            
            ###BUY###
            for ask, vol in sell_orders.items():
                if max_conversion > 0 and (conversion_bid - conversion_sell_costs > best_ask) and position < 0:
                    conv = min((-vol), max_conversion)
                    conversions += conv
                    max_conversion -= conv
                    vol += conv
                    print("Conversion details:", "Product:", product, "Conv_bid:", conversion_bid, " Costs:  ",  conversion_buy_costs, "Best ask: ", best_ask, "Amount:", conv)
            ...
            for bid, vol in buy_orders.items():
                if max_conversion > 0 and (conversion_ask + conversion_buy_costs < best_bid) and position > 0:
                    conv = min(vol, max_conversion)
                    conversions += conv
                    max_conversion -= conv
                    vol -= conv
                    print("Conversion details:", "Product:", product, "Conv_ask:", conversion_ask, " Costs:  ",  conversion_sell_costs, "Best bid: ", best_bid, "Amount:", conv)
